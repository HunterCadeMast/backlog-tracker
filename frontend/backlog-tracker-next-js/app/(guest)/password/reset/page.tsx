        ...
        <div className = "h-full flex items-center justify-center bg-main-compliment">
                <div className = "flex flex-col items-center gap-6">
                    <form onSubmit = {handleSubmit} className = "w-90 p-8 rounded-2xl shadow-2xl bg-ui">
                        <h1 className = "text-5xl sm:text-7xl text-white font-log-title mb-3">Password Reset</h1>
                        {message && <p className = "text-3xl text-white mb-3">{message}</p>}
                        {error && <p className = "text-3xl text-red-500">{error}</p>}
                        <input type = "email" value = {email} placeholder = "Email" onChange = {x => setEmail(x.target.value)} className = "btn w-full mb-5 p-3" required />
